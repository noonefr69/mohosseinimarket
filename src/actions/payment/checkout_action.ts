...
export async function CheckoutAction(items: ClientCartItem[]) {
  try {
    ...
    for (const dbProduct of dbProducts) {
      ...
      // Use the REAL price from the database
      const itemTotal = dbProduct.price * quantity;
      totalPrice += itemTotal;

      // Snapshot the current DB state into the order
      orderItems.push({
        productId: dbProduct._id,
        name: dbProduct.name, // Snapshot
        price: dbProduct.price, // Snapshot (Real DB price)
        quantity: quantity,
        unit: dbProduct.unit,
    ...
    const order = await Order.create({
      userId: user._id,
      items: orderItems, // Stores the SNAPSHOTS
      totalPrice, // Calculated securely on the server
      status: "pending",
